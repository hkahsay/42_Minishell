...
static int ft_wordlen(char *input_str, char *q)
{
    int i = 0;
    ...
    while (input_str[i] != '\0' && input_str[i] != ' ' && input_str[i] != '\t' && input_str[i] != '\n')
    {
        i++;
        printf(MAGENTA "\n0word_len %d\n", i);
        printf(TEAL "\nword_len *input_str %c\n", *input_str);
        if ((ft_strncmp(input_str, q, ft_strlen(q)) == 0 && quote_status == CLOSED)) // if CLOSED quote is found with char
        {
            i++;
            printf(MAGENTA "\n1word_len %d\n", i);
            quote_status = OPEN;
            printf(TEAL "\nword_len2: found open quote: %d *str: %c\n" RS, quote_status, *input_str); 
            printf(TEAL "\n1 left here\n" RS);    
        }
        if ((*input_str != '\0'  && quote_status == OPEN && (*input_str == ' ' || *input_str == '\t' || *input_str == '\n'))) //if OPENED quote found with space
            break ;
        printf("word_len ft_strncmp: %d *input_str %c, ft_strlen(q) %zu\n", ft_strncmp(input_str, q, ft_strlen(q)), *input_str, ft_strlen(q));
        if ((*input_str != '\0'  && quote_status == OPEN && (input_str[i] != '\0' && input_str[i] != ' ' && input_str[i] != '\t' && input_str[i] != '\n'))) //if OPENED quote char found
        {
            i++;
            printf(MAGENTA "\n2word_len %d\n", i);
        }     
        if ((ft_strncmp(input_str, q, ft_strlen(q)) == 0 && quote_status == OPEN))  //if OPENED quote found with char
        {
            i++;
            printf(MAGENTA "\n3word_len %d\n", i);
            quote_status = CLOSED;
            printf(TEAL "\n2 left here\n" RS);
            printf(R "I exit the word_len\n" RS);
            return (i);
        }
		else
			i++;
...
static char    *word_dupe(char *input_str, char *quote)
{
    int i = 0;
    int len = ft_wordlen(input_str, quote);
    ...
    printf(R "Ready to return word\n" RS);
    return (word);
}

static void    fill_words(char **array, char *input_str, char *q)
{
    int word_index = 0;
    // int quote_status = CLOSED;
    ...
    while (*input_str != '\0')
    {
        array[word_index] = word_dupe(input_str, q);
        ++word_index;
        ...
        //     while (*input_str != '\0' && *input_str != ' ' && *input_str != '\t' && *input_str != '\n')
        //     {
        //         printf("fill_words: *input_str in while loop 1: %c\n", *input_str);
        //         if ((ft_strncmp(input_str, q, ft_strlen(q)) == 0 && quote_status == CLOSED)) // && *(input_str + 1) != '\0'
        //         {
        //             quote_status = OPEN;
        //             input_str++;
        //             printf("\nfill_words: 2:found open quote: %d *str: %c\n", quote_status, *input_str); 
        //             printf(LIME "fill_words: 1 left here\n" RS);    
        //         }
        //         if ((*input_str != '\0'  && quote_status == OPEN && (*input_str == ' ' || *input_str == '\t' || *input_str == '\n')))
        //             break;
        //         printf("fill_words: ft_strncmp: %d *input_str %c, ft_strlen(q) %zu\n", ft_strncmp(input_str, q, ft_strlen(q)), *input_str, ft_strlen(q)); 
        //         if ((ft_strncmp(input_str, q, ft_strlen(q)) == 0 && quote_status == OPEN))
        //         {
        //             quote_status = CLOSED;
        //             input_str++;
        //             printf(BLUE "fill_words: 2 left here\n" RS); 
        //             break ;
        //         }
        //         else
        //             input_str++;
        ...
        //         if (quote_status == OPEN)
        //         {
        //             printf("\nfill_words: 7:found open quote: %d *str: %c\n", quote_status, *input_str);
        //             if ((*input_str != '\0' && (*input_str == ' ' || *input_str == '\t' || *input_str == '\n')))
        //             {
        //                 printf(DB "fill_words: I counted empty spaces in quotes\n" RS);
        //                 input_str++;
        //             }
        //             if ((*input_str != '\0'  && quote_status == OPEN && (*input_str != '\0' && *input_str != ' ' && *input_str != '\t' && *input_str != '\n')))
        //                 break;  
        //         }
        //         else
        //             input_str++;
        //         printf(R "fill_words: I was here in the quotes\n" RS);
        //     }
        //     printf(R "end fill_words: *input_str in while loop 1: %c\n" RS, *input_str);
        // }
    }
}
...
static int     count_words(char *input_str, char *q)
{
	...
	if (quote_status == OPEN && (ft_strncmp(input_str, q, ft_strlen(q)) != 0 && *input_str == '\0'))
    {
        printf("error");
        return (-1);
    }
...
char    **split_line(char *input_str)
{
    ...
    if (!array)
        return (NULL);
    array[word_count] = 0;
    fill_words(array, input_str, quote);
    int i = 0;
    while (array[i])
    {
        printf("%s\n", array[i++]);
